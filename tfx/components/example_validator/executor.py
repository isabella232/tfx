...
class Executor(base_executor.BaseExecutor):
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    exclude_splits = json_utils.loads(
        exec_properties.get(component_specs.EXCLUDE_SPLITS_KEY, 'null')) or []
    ...
    stats_artifact = artifact_utils.get_single_instance(
        input_dict[component_specs.STATISTICS_KEY])
    ...
    anomalies_artifact = artifact_utils.get_single_instance(
        output_dict[component_specs.ANOMALIES_KEY])
    ...
    schema = io_utils.SchemaReader().read(
        io_utils.get_only_uri_in_dir(
            artifact_utils.get_single_uri(
                input_dict[component_specs.SCHEMA_KEY])))
    ...
    for split in artifact_utils.decode_split_names(stats_artifact.split_names):
      ...
      label_inputs = {
          ...
          component_specs.STATISTICS_KEY:
              tfdv.load_statistics(
                  io_utils.get_only_uri_in_dir(
                      os.path.join(stats_artifact.uri, split))),
          component_specs.SCHEMA_KEY:
              schema
          # labels.SCHEMA:
          #     schema
      }
      output_uri = artifact_utils.get_split_uri(
          output_dict[component_specs.ANOMALIES_KEY], split)
  ...
  def _Validate(self, inputs: Dict[Text, Any], outputs: Dict[Text,
    ...
    Args:
      inputs: A dictionary of labeled input values, including:
        - component_specs.STATISTICS_KEY: the feature statistics to validate
        - component_specs.SCHEMA_KEY: the schema to respect
    ...
    """
    schema = value_utils.GetSoleValue(inputs, component_specs.SCHEMA_KEY)
    stats = value_utils.GetSoleValue(inputs, component_specs.STATISTICS_KE)
